...
@Component
public class MatchFoundHandler implements MessageHandler {
    
    private static final Logger logger = LoggerFactory.getLogger(MatchFoundHandler.class);
    ...
    public void handleMessage(WebSocketMessage message, SimpMessagingTemplate messagingTemplate) {
        try {
            ...
            logger.error("Erro ao processar match: {}", e.getMessage());
        }
    }
    
    @Override
    public boolean supports(WebSocketMessage message) {
        return message.getMessageType() == WebSocketMessageType.MATCH_FOUND;
    }
    
    @Override
    public String getHandlerName() {
        return "MatchFoundHandler";
    }
}
